        ...
        {!refreshTokenTaskId ? (
          <>
            <Alert
              ...
              description="按代价从低到高试：先用 refresh_token 换新的 access_token（顺带换新 RT）；不行再用 session_token 换（先打 /api/auth/session 回放，捞不到就恢复会话走一遍授权链换全新 AT + RT）；都没有或都失效时，用邮箱密码走一遍协议登录（库里存了 2FA 密钥会自动算码），可能需要收一封验证码。刷到的 Token 会直接写回账号。"
            />
